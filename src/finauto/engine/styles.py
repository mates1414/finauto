...
class Styles:
    def __init__(self, wb: xlsxwriter.Workbook):
        ...
        self.title = wb.add_format(
            {**base, "bold": True, "font_size": 14, "font_color": "#1F4E78"}
        )
        self.section = wb.add_format(
            {**base, "bold": True, "font_color": "#FFFFFF", "bg_color": "#1F4E78"}
        )
        self.colhead = wb.add_format(
            {**base, "bold": True, "bottom": 1, "align": "center"}
        )
        self.year_head = wb.add_format(
            {**base, "bold": True, "align": "center", "bottom": 1, "num_format": "0"}
        )
        ...
        # Blue-on-cream input cells are the edit surface. `locked: False` leaves
        # them editable when the sheet is protected (see WorkbookBuilder); every
        # other cell stays locked, so users edit only intended inputs and the
        # formula network cannot be broken by accident (phase3.md A4 hardening).
        input_base = {
            **base,
            "font_color": "#0070C0",
            "bg_color": "#FFF7E6",
        ...
        self.num_bold = wb.add_format(
            {**base, "bold": True, "top": 1, "num_format": "#,##0;[Red](#,##0)"}
        )
        self.pct = wb.add_format({**base, "num_format": "0.00%"})
        self.pct_bold = wb.add_format(
            {**base, "bold": True, "top": 1, "num_format": "0.00%"}
        )
        self.price = wb.add_format({**base, "num_format": "#,##0.00"})
        self.price_bold = wb.add_format(
            {**base, "bold": True, "top": 1, "num_format": "#,##0.00"}
        )
